...
class Command(BaseCommand):
    ...
    @transaction.atomic
    def fill_users(self, cnt=10000):
        user_names = set()
        while len(user_names) < cnt:
            user_names.add(self.f.user_name() + self.f.user_name()[-3:])
        ...
        for i in range(cnt):
            ...
            Profile.objects.create(
                user=user,
                nick_name=self.f.word()[:50],
            )

    @transaction.atomic
    def fill_questions(self, cnt=100000):
        authors = Profile.objects.all()
        tags = Tag.objects.all()
        for i in range(cnt):
            question = Question.objects.create(
                author=choice(authors),
                text='. '.join(self.f.sentences(self.f.random_int(min=2, max=5))),
                title=self.f.sentence()[:128],
            )
            for _ in range(randint(0, 4)):
                question.tags.add(choice(tags))

    @transaction.atomic
    def fill_answers(self, cnt=1000000):
        authors = Profile.objects.all()
        ...
        for i in range(cnt):
            Answer.objects.create(
                ...
                text='. '.join(self.f.sentences(self.f.random_int(min=2, max=5))),
                flag_correct_answer=self.f.pybool(),
            )
    ...
    def fill_tags(self, cnt=10_000):
        tags = set()
        while len(tags) < cnt:
            tags.add(self.f.word()[:10] + self.f.word()[:10])
